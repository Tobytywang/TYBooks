...
echo "=== TYBooks Standby 初始化 ==="

if [ -z "${PRIMARY_IP}" ] || [ -z "${REPLICATOR_PASSWORD}" ] || [ -z "${POSTGRES_PASSWORD}" ]; then
  echo "错误：请先设置环境变量 PRIMARY_IP, REPLICATOR_PASSWORD, POSTGRES_PASSWORD"
...
docker compose run --rm -v pg-data:/var/lib/postgresql/data postgres sh -c \
  "echo \"primary_conninfo = 'host=${PRIMARY_IP} port=5432 user=replicator password=${REPLICATOR_PASSWORD}'\" >> /var/lib/postgresql/data/postgresql.auto.conf"
